...
- [Proof-aware Pair client boundary](issues/27-implement-proof-aware-pair-client-boundary.md): closed Pair proof admission, stable atomic command generations, input-gated bounded role turns, event-local stopped submissions, presentation-only Remote Avatar snap acknowledgement, and immutable revision sidecars are implemented and covered by the Pair readiness smoke.
