...
- [Predict and Submit Bounded Ground Movement](issues/17-predict-submit-movement.md) — The retained live session now predicts bounded heading-aligned movement, submits valid start/heartbeat/stop frames, keeps client-written and realm-observed pose truth separate, and records deterministic plus reset-scoped Metal evidence without claiming realm acceptance or persistence.
- [Prove Persisted Movement and Recover Explicitly](issues/18-prove-persisted-movement.md) — The client proves one bounded submitted stop only through a fresh saving reconnect, keeps proof input frozen across the lifecycle, exercises the short-movement and local Worldserver-outage negative probes, and restores Realm health without database-derived success claims.
- [Harden and Accept the World-entry Candidate](issues/19-harden-world-entry-acceptance.md) — Candidate `15d5839` passed the complete deterministic, platform, live Realm, and manual acceptance contract once; the retained bundle hash-binds each machine PASS marker, canonical semantic evidence, and the SHA-bound attestation while explicitly deferring Windows runtime acceptance.
